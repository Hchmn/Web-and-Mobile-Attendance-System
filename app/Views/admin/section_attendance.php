    ...
    <div id="content" class="p-4 p-md-5">
      ...
      <div class="table-responsive  ">
        <table class="table table-hover">
          ...
          <tbody id="tableBody">
            <?php for($x = 0; $x < sizeof($attendanceData); $x++) {
              $studentId = $attendanceData[$x][0];
              $ID = $attendanceData[$x][1];
              $studentFNAME = $attendanceData[$x][2];
              $studentLNAME = $attendanceData[$x][3];
              $studentAGE = $attendanceData[$x][4];
              $studentGENDER = $attendanceData[$x][5];
              $absences = $attendanceData[$x][7];
              $present = $attendanceData[$x][6];
              $total_attendance = $attendanceData[$x][8];       
            ?>
              <tr>
                <td><?php echo $ID; ?></td>
                ...
                <td>
                  <a href="/admin_section_student_attendance<?php echo "/".$studentId?>" class="btn btn-success">
                      View
                  </a>
                </td>
              </tr>
